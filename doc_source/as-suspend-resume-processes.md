...
`ReplaceUnhealthy`
+ Amazon EC2 Auto Scaling stops replacing instances that are marked as unhealthy\. Instances that fail EC2 or Elastic Load Balancing health checks will still be marked as unhealthy\. As soon as you resume the `ReplaceUnhealthly` process, Amazon EC2 Auto Scaling replaces instances that were marked unhealthy while this process was suspended\. The `ReplaceUnhealthy` process calls both of the primary process types—first `Terminate` and then `Launch`\. 
