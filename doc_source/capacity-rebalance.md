...
+ We also recommend that you use the `capacity-optimized` allocation strategy\. Used in combination with the Capacity Rebalancing feature, the `capacity-optimized` allocation strategy ensures that your instances are maintained in the most optimal Spot pools\. 
...
+ You can configure a lifecycle hook when enabling Capacity Rebalancing to gracefully shut down your application inside the instances that receive the rebalance notification, before Amazon EC2 Auto Scaling terminates the instances\. 
+ When using Spot Instances, your application should be tolerant of some interruption to prevent disruptions to your application\. In cases where an instance receives a final two\-minute interruption notice, Amazon EC2 Auto Scaling calls the terminating lifecycle hook and attempts to launch a replacement immediately\.
...
1. To define a lifecycle hook, do the following:
   ...
   1. \(Optional\) For **Notification metadata**, specify additional information that you want to include anytime that Amazon EC2 Auto Scaling sends a message to an AWS Lambda function or another target that you configure in step 7\. 
